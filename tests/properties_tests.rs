...
fn properties_support_nested_dictionaries() {
    ...
    root.set_string("ProfileName", "Display P3").expect("set profile");
    let properties = root.freeze().expect("freeze root properties");

    assert_eq!(properties.string("ProfileName").expect("read profile"), Some("Display P3".into()));
    let nested = properties.dictionary("{PNG}").expect("read nested dictionary").expect("nested png");
